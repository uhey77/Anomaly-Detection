...
from config import (
    ANOMALY_METHOD,
    ANOMALY_THRESHOLD,
    ENABLE_CROSSCHECK_AGENT,
    ENABLE_KNOWLEDGE_AGENT,
    ENABLE_MANAGER_AGENT,
    ENABLE_REPORT_AGENT,
    ENABLE_WEB_AGENT,
    END_DATE,
    LLM_PROVIDER,
...
from utils.data_utils import load_sample_data, plot_anomalies, save_sample_data
...
def main():
    ...
    plot_anomalies(
        df,
        anomalies,
        output_file=f"anomalies_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png",
    )
...
def save_results(anomalies, agent_findings):
                ...
                for key, value in data.items():
                    if isinstance(value, JSON_SCALAR_TYPES):
                        ...
                        for sub_key, sub_value in value.items():
                            if isinstance(sub_value, JSON_SCALAR_TYPES):
    ...
    with open(f"results/report_{timestamp}.txt", "w", encoding="utf-8") as f:
        ...
        if (
            "管理者エージェント" in agent_findings
            and "findings" in agent_findings["管理者エージェント"]
        ):
            f.write("## 最終評価\n\n")
